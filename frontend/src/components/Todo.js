...
function Todo() {
    ...
    let deleteTodo = async (id, isCompleted) => {
        try {
            const updateTodo = await axios.patch(`${BASE_URL}/todos/status/${id}`, { isCompleted })
    ...
    let dragDrop = async (id) => {
        try {
            const updateTodo = await axios.patch(`${BASE_URL}/todos/${id}`, { prePosition, curPosition })
            setTodoList(updateTodo.data)
        } catch (error) {
            alert(error)
        }
        ...
        <>
            <div className="relative p-10 rounded-md shadow-sm container mx-auto px-4 border-2 ">
                <div className="w-full flex justify-center">
                    <span className="text-lg font-large text-slate-700 mr-6" >Title : </span>
                    <input type="text"
                        name="todo"
                        id="todo"
                    ...
                    <input type="text"
                        ...
                        id="todo"
                        className=" w-100 me-5 rounded-md border-0 py-1.5 pl-7 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 lg:text-lg lg:leading-1"
                        placeholder="enter Url..." onChange={(e) => { setLink(e.target.value) }}></input>
                    <button className="w-100 py-1.5 pl-7 pr-7 bg-violet-500 hover:bg-violet-600 active:bg-violet-700 focus:outline-none focus:ring focus:ring-violet-300" onClick={addTodo}>
                        add
                    </button>
                </div>
                                ...
                                {item.isCompleted ?
                                    <s>
                                        <div className="ml-6 overflow-hidden">
                                            ...
                                            <p className="text-lg text-slate-500 truncate">{item.link}</p>
                                        </div>
                                    </s> : <div className="ml-6 overflow-hidden">
                                        <p className="text-lg font-medium text-slate-900">{item.title}</p>
                                        <p className="text-lg text-slate-500 truncate">{item.link}</p>
                                    </div>
